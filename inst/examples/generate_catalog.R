...
s3 <- arrow::s3_bucket(bucket = paste0("neon4cast-forecasts/parquet/",theme,'/model_id=',model_id, '/reference_datetime=',reference_datetime),  ## this will speed up if you use above path for forecast (subset within path)
                       endpoint_override = "data.ecoforecast.org",
                       anonymous = TRUE)


theme_df <- arrow::open_dataset(s3) %>%
  filter(variable == variable_name, site_id == site_id)#, model_id == 'flareGLM') #%>% ##this used to just download data as quickly as possible -- need to revisit
#collect()
